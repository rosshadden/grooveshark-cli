...
}, ["play", "pause", "restore", "next", "prev", "stop", "clear", "status",
    "volume", "mute", "queue", "start-server", "stop-server"]);
...
cli.main(function(args, options) {
  ...
  // when the socket connects to the server, fire off the command we passed in. 
  // The socket server will issue the disconnect. The reason it doesn't disconnect
  // right away is because `gs-status` and `gs-queue` need to wait and get a response
  // back from the server and display that response in the terminal.
  if (command.is("play", "pause", "restore", "next", "prev", "stop", "volume", "mute",
                 "clear", "status", "queue")) {
